...
- **P5 implementation repaired, fresh review required**: the first accepted-closeout attempt exposed a
  lifecycle regression-test defect because the suite hard-coded SKY-026's active `planning/projects/`
  location and therefore failed when the directive moved to `planning/archive/`. The test now follows
  exactly one active-or-archived SKY-026 location and guards against duplicate copies. Because that test
  repair is substantive, the prior ACCEPT is stale and P5 remains unaccepted until another fresh review.
...
The previous ACCEPT is stale because closeout required a substantive lifecycle-test repair. Finish green
CI on the repaired #253 head, then run one fresh review of **open PR #253**. On new ACCEPT, return to the
original session with only `accepted`; it will rerun bounded same-PR closeout and hand #253 back for one
human merge.
