...
### Phase 10 — final independent close-out correction  (~1h)  `[ ]`

A final independent audit of `main` at `f6bd03e` found false negatives in the temporal-hygiene gate
and one proof-only deploy target that survived the compatibility audit. Keep this phase T1-only and do
not mutate live infrastructure or authority boundaries.

1. Expand temporal-hygiene current-surface discovery to include `.githooks/pre-commit` and
   `.github/workflows/*.yml` in addition to the existing operational surfaces.
2. Purge the remaining high-confidence current-state archaeology found by the independent audit,
   including at minimum:
   - `bin/ops`: the removed-flag replacement story;
   - `hosts/lxc-proof/default.nix`: throwaway/proof/phase narration, the stale
     `hosts/lxc-adguard-network/` path, and the "historically-broken" proof marker explanation;
   - `compose/obsidian-livesync/local.ini`: "reviewed once" and historical pain narration;
   - `nix/modules/ops-user.nix`: Before/After migration narration, "now"/"removed" chronology,
     later-phase language, and proof anecdotes;
   - `docs/conventions/docs.md`: the old `tokens:` frontmatter/lint-gate story;
   - `README.md`: rehearsal chronology around the kill-switch guarantee;
   - `scripts/cf-dns-route.sh`: "now TOFU" chronology while retaining its current break-glass role;
   - `.githooks/pre-commit`, `.github/workflows/checks.yml`, and `.github/workflows/nix.yml`:
     completed directive/phase provenance in live gate wiring.
3. Strengthen the temporal-hygiene fixtures and matching for the exact false-negative classes found
   here, such as `replaces`, `historically`, comment/prose Before/After migration narration, and
   proof/phase wording. Do not globally ban runtime words whose present meaning is legitimate; prefer
   rewriting current artifacts and use narrowly scoped patterns/fixtures.
4. Re-audit the `lxc-proof` surface by behavior. Unless a current non-history consumer is demonstrated,
   remove `deploy.nodes.lxc-proof` because it targets the old proof address `10.10.90.99` with no
   current guest/runbook consumer. Retain `nixosConfigurations.lxc-proof`, the `lxc-proof-tarball`
   build, and the uploaded `local:vztmpl/nixos-lxc-proof-26.05.tar.xz` filename while current OpenTofu
   declarations consume that template. Remove the proof-only marker package/comment if it has no
   current bootstrap purpose. Do not rename or replace the uploaded template in this phase.
5. Preserve current compatibility/recovery behavior: `scripts/collect-firewall.sh` as the offline/DR
   parser, `scripts/cf-dns-route.sh` as break-glass/delete checkpoint, and functional OpenTofu moved
   blocks/import ignores. `envsync.sh` remains deleted.
6. Run the full pre-commit-equivalent suite, `bin/ops hygiene`, `git diff --check`, shell syntax checks,
   renderer/catalog consistency, `nix flake check --no-write-lock-file`, and the relevant Nix build
   gate. Manually review every remaining temporal search hit; zero regex hits alone are not acceptance.
7. Repeat the cold-agent route review and compare the result to `docs/system-design.md`. Only after
   those checks pass may the directive be marked complete and archived again.

**Exit:**

- the gate covers hooks/workflows and rejects the newly demonstrated false-negative classes with
  focused regression fixtures while legitimate current compatibility language still passes;
- the manually reviewed current-authority corpus contains no unnecessary project/proof chronology or
  stale operational path;
- the dead `lxc-proof` deploy target is removed or has a demonstrated current consumer, while the
  template compatibility needed by OpenTofu remains explicit;
- full validation is green and no trust, credential, firewall, recovery, infrastructure-authority, or
  autonomy boundary changed;
- the final close-out journal records independent evidence, then the supported planning lifecycle
  archives SKY-023 with exactly one directive copy.
...
- 2026-09-06 — final independent close-out audit at `main` `f6bd03e` found live temporal archaeology
  outside the gate's effective coverage plus a proof-only `deploy.nodes.lxc-proof` target. SKY-023 is
  reopened at Phase 10 for one bounded corrective checkpoint; no live system or authority boundary was
