...
## 0. Workspace
Laptop + phone hotspot. Clone **both** repos (`skynet`, `skynet-opnsense-backup`). Run any
...
2. Redo NIC passthrough from the documented PCI IDs (`runbooks/dr/` / survival kit).
3. **System → Configuration → Restore** with `config.xml` from `skynet-opnsense-backup`.
