...
- An external ACCEPT records the reviewer-resolved base+head pair: current base/main SHA and PR head SHA verified immediately before verdict. Known movement of either before merge makes it stale; GitHub mergeability or unchanged head alone is not evidence that the reviewed integration result is unchanged.
