...
| [`deploy-service.md`](deploy-service.md) | T2 PR-gated | Deploy or update a service | Deploy or update a service: edit compose, PR with the dry-run effect, merge; skynet deploy applies, verifies, and rolls back by itself. |
...
| [`diagnose/deploy-stuck.md`](diagnose/deploy-stuck.md) | T1/T2 | A merged compose PR didn't deploy / a deploy rolled back / git and running have drifted | Triage a merged compose PR that isn't running — read the operation record and host facts, then classify: held, refused, rolled back, or drifted. |
