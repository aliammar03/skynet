...
| IP | Host / alias | Source | Notes |
|----|--------------|--------|-------|
| 10.10.20.15 | vm-docker-core | dhcp | Core Docker VM |
| 10.10.20.20 | vm-unraid-nas | dhcp | Unraid |
| 10.10.20.30 | lxc-adguard-network | dhcp | Adguard-Network |
| 10.10.20.31 | lxc-adguard-core | dhcp | Adguard-Core |
| 10.10.20.35 | lxc-caddy | dhcp | Caddy |
| 10.10.20.37 | lxc-apt-cacher-ng | dhcp | Apt-Cacher-NG |
| 10.10.20.40 | HOST_PBS | alias | Proxmox Backup Server at its current VLAN 20 address. |
| 10.10.20.63 | ROLE_APP_ORIGINS | alias | Application origin hosts reachable only from the DMZ reverse proxy on approved backend ports. |
| 10.10.20.75 | lxc-karakeep | dhcp | Karakeep |
...
| IP | Host / alias | Source | Notes |
|----|--------------|--------|-------|
| 10.10.40.15 | vm-docker-dmz | dhcp | Network DMZ VM |
| 10.10.40.33 | lxc-cloudflared | dhcp | Cloudflared |
...
| 10.10.50.15 | vm-docker-network | dhcp | Network Docker VM |
| 10.10.50.25 | lxc-omada | dhcp | Omada Controller |

## VLAN 60 — Trusted LAN  (10.10.60.0/24)

| IP | Host / alias | Source | Notes |
|----|--------------|--------|-------|
| 10.10.60.35 | HOST_PROXY_ADMIN | alias | Management Caddy front door in VLAN 60 Admin Access. |

## VLAN 70 — DNS & Services  (10.10.70.0/24)
...
| 10.10.90.99 | HOST_SKYNET_OPS | alias | skynet-ops agent VM 9090 |
...
| 10.10.100.53 | ROLE_APP_ORIGINS | alias | Application origin hosts reachable only from the DMZ reverse proxy on approved backend ports. |
| 10.10.100.65 | ROLE_USENET_CLIENTS | alias | Hosts permitted outbound NNTP and NNTPS access through PORT_USENET_OUT. |
| 10.10.100.66 | ROLE_APP_ORIGINS | alias | Application origin hosts reachable only from the DMZ reverse proxy on approved backend ports. |
| 10.10.100.69 | ROLE_DMZ_AI_CLIENTS | alias | DMZ AI clients permitted to reach approved AI and ComfyUI services. |
| 10.10.100.75 | ROLE_APP_ORIGINS | alias | Application origin hosts reachable only from the DMZ reverse proxy on approved backend ports. |
