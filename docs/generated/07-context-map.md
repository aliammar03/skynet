...
**Cold-boot read:** `docs/generated/06-agent-digest.md` ≈ 1557 tok.
...
| `runbooks/nightly.md` | T1 read + generated-only PR | Run the nightly / nightly timer | 1588 | The report-only nightly maintenance run on both engine paths, and what it refreshes. |
...
| `docs/design/observability.md` | 967 | How machine state becomes human-readable docs, and how the nightly run keeps the picture current. |
...
| `docs/generated/06-agent-digest.md` | 1557 | Agent Digest |
...
- `journal/` — 90 raw episodes, ≈ 101737 tok total. Retrieve by topic: `bin/recall <topic>` or `grep -ri "<topic>" journal/`; recent episodes are already in `06-agent-digest.md`. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **50694** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
