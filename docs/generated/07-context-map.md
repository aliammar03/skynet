...
| `planning/README.md` | 2064 | Where future work lives as SKY-### directives: the scratchpad→ideas→backlog→projects→archive lifecycle, bin/plan, and the roadmap. |
...
| `docs/generated/06-agent-digest.md` | 1555 | Recent-activity, episodic, and open-thread retrieval view. |
...
- `journal/` — 126 raw episodes, ≈ 145769 tok total. Retrieve by topic: `bin/recall <topic>` or `grep -ri "<topic>" journal/`; use `06-agent-digest.md` when recent activity, open threads, or episode pointers are useful. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **56834** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
