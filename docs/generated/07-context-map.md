...
**Always-loaded contract baseline:** `AGENTS.md` + `CLAUDE.md` ≈ **3798** tok — never in this list.
...
| `planning/README.md` | 1864 | Where future work lives as SKY-### directives: the scratchpad→ideas→backlog→projects→archive lifecycle, bin/plan, and the roadmap. |
...
| `docs/generated/06-agent-digest.md` | 1359 | Recent-activity, episodic, and open-thread retrieval view. |
...
- `journal/` — 134 raw episodes, ≈ 153879 tok total. Retrieve by topic: `skynet recall --repo <checkout> <topic>` or `grep -ri "<topic>" journal/`; use `06-agent-digest.md` when recent activity, open threads, or episode pointers are useful. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **51480** tok across 52 files — but you load a *row* (≈ tens of tok) to choose, then one file.
