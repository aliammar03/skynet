...
**Always-loaded baseline:** `AGENTS.md` + `CLAUDE.md` ≈ **3769** tok — the contract; never in this list.
**Cold-boot read:** `docs/generated/06-agent-digest.md` ≈ 1293 tok.
...
| `docs/generated/06-agent-digest.md` | 1293 | Agent Digest |
...
- `journal/` — 77 raw episodes, ≈ 84811 tok total. Retrieve by topic: `bin/recall <topic>` or `grep -ri "<topic>" journal/`; recent episodes are already in `06-agent-digest.md`. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **49818** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
