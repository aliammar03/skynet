...
| `docs/generated/06-agent-digest.md` | 1541 | Recent-activity, episodic, and open-thread retrieval view. |
...
- `journal/` — 129 raw episodes, ≈ 148617 tok total. Retrieve by topic: `skynet recall --repo <checkout> <topic>` or `grep -ri "<topic>" journal/`; use `06-agent-digest.md` when recent activity, open threads, or episode pointers are useful. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **56992** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
