...
**Cold-boot read:** `docs/generated/06-agent-digest.md` ≈ 1575 tok.
...
| `docs/design/observability.md` | 1240 | How machine state becomes human-readable docs, and how the nightly run keeps the picture current. |
...
| `docs/generated/06-agent-digest.md` | 1575 | Agent Digest |
...
- `journal/` — 101 raw episodes, ≈ 117604 tok total. Retrieve by topic: `bin/recall <topic>` or `grep -ri "<topic>" journal/`; recent episodes are already in `06-agent-digest.md`. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **51151** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
