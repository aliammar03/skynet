...
**Cold-boot read:** `docs/generated/06-agent-digest.md` ≈ 1092 tok.
...
| `runbooks/construction-delegation.md` | T1 build-time only | Do a substantial construction task / build X / implement or change X | 695 | Run substantial construction as a lead — route bounded helpers, verify their work, and open the PR without granting production authority. |
...
| `runbooks/nightly.md` | T1 read + generated-only PR | Run the nightly / nightly timer | 1272 | The report-only nightly maintenance run on both engine paths, and what it refreshes. |
...
| `runbooks/restore-service.md` | T2; PBS token for VM restore | Restore a service / recover from backup | 739 | Restore a service or VM from restic/PBS using a selected recovery point. |
...
| `planning/README.md` | 1586 | Where future work lives as SKY-### directives: the scratchpad→ideas→backlog→projects→archive lifecycle, bin/plan, and the roadmap. |
...
| `docs/generated/06-agent-digest.md` | 1092 | Agent Digest |
...
- `journal/` — 68 raw episodes, ≈ 76660 tok total. Retrieve by topic: `bin/recall <topic>` (SKY-010 P4) or `grep -ri "<topic>" journal/`; recent episodes are already in `06-agent-digest.md`. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **50880** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
