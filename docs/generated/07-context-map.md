...
**Always-loaded contract baseline:** `AGENTS.md` + `CLAUDE.md` ≈ **4439** tok — never in this list.
...
| `docs/design/actuators.md` | 597 | The current write actuators, deterministic rollback paths, and A4 eligibility of each capability. |
...
| `docs/design/memory.md` | 599 | How Skynet keeps portable semantic, procedural, episodic, and working memory without overloading a fresh agent. |
...
| `docs/design/observability.md` | 1389 | How machine state becomes human-readable docs, and how the nightly run keeps the picture current. |
...
- `journal/` — 127 raw episodes, ≈ 146526 tok total. Retrieve by topic: `skynet recall --repo <checkout> <topic>` or `grep -ri "<topic>" journal/`; use `06-agent-digest.md` when recent activity, open threads, or episode pointers are useful. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **56909** tok across 53 files — but you load a *row* (≈ tens of tok) to choose, then one file.
