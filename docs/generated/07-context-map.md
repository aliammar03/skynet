...
**Always-loaded baseline:** `AGENTS.md` + `CLAUDE.md` ≈ **3302** tok — the contract; never in this list.
**Cold-boot read:** `docs/generated/06-agent-digest.md` ≈ 1453 tok.
...
| Path | ~tok | Summary |
|---|--:|---|
| `docs/design/access-and-trust.md` | 4320 | The trust tiers in full — every token, ACL, principal, and the auto-expiring SSH root grant Skynet can request but never mint. |
...
| `planning/README.md` | 1469 | Where future work lives as SKY-### directives: the scratchpad→ideas→backlog→projects→archive lifecycle, bin/plan, and the roadmap. |
| `runbooks/README.md` | 1500 | Catalog of engine-neutral procedures any agent can execute, each tagged by tier and trigger — the routing menu. |
...
| `docs/generated/05-state-of-the-lab.md` | 1532 | State of the Lab |
| `docs/generated/06-agent-digest.md` | 1453 | Agent Digest |
...
| `docs/generated/90-backup-status.md` | 479 | Backup & grant status |
| `docs/generated/README.md` | 227 | Skynet — generated docs |

## Episodic memory — retrieve by topic, don't browse

- `journal/` — 40 raw episodes, ≈ 48481 tok total. Retrieve by topic: `bin/recall <topic>` (SKY-010 P4) or `grep -ri "<topic>" journal/`; recent episodes are already in `06-agent-digest.md`. **Do not load the whole store.**

---
**On-demand corpus:** ≈ **52115** tok across 41 files — but you load a *row* (≈ tens of tok) to choose, then one file.
