...
|---|---|---|---|---|
| Compose deploy | `gitops-deploy.sh` (optional `--gate` invokes the packaged verifier) | `gitops-rollback.sh --prepare` creates a reviewed inverse; no automatic authored revert | `skynet verify deployment <service> <full-revision>` report | No |
