...
Main reads the Direct set once. Companion owns the initial bulky/reusable intake, then later checks only
changed facts or conflicts; Investigator owns bounded unfamiliar external evidence. The map is working
state, not durable documentation; reclassify only when evidence changes relevance. Do not directly
explore a Companion/Investigator surface unless it becomes decision-critical. `[manual]`
...
owning tools. For work crossing sessions, keep a compact ignored `.agent/CHECKPOINT.md` only as
disposable working state and delete it once durable facts reach their canonical home. `[manual]`

Every substantive closure has exactly one next-session entry point in the active directive: `[manual]`

| State | Durable closure | Next-session entry point |
|---|---|---|
| complete | Main accepts, advances directive state, appends the raw journal episode, and regenerates owned views | the directive's next-phase Execute/Continue prompt |
| paused | Main records the verified position and unmet work without advancing the phase; checkpoint only if useful | the directive's same-phase Continue prompt |
| blocked | Main sets directive status `blocked`, records the exact external condition, and appends the raw episode | the directive's same-phase Continue prompt naming the unblock condition |
