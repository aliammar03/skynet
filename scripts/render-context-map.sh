#!/usr/bin/env bash
# render-context-map.sh — generate docs/generated/07-context-map.md, the on-demand "what can I load
# and what does it cost" index. It lists one row per loadable artifact (path · tier · trigger ·
# ~tokens · summary) so a session can route to the exact file it needs instead of loading a whole
# prose catalog. This is the machine-readable half of the default-lean discipline
# (docs/design/memory.md); it is not a fresh-session continuity contract.
#
...
mkdir -p "$(dirname "${PAGE}")"
{
  ...
  printf -- '**Always-loaded contract baseline:** `AGENTS.md` + `CLAUDE.md` ≈ **%s** tok — never in this list.\n' "$base"
  printf 'Use this map after normal continuity intake (`agent_docs/` + the active directive) to route\n'
  printf 'additional reads. Everything below is **on-demand**: open a *file*, not a section.\n\n'
  ...
  printf -- '- `journal/` — %s raw episodes, ≈ %s tok total. Retrieve by topic: `bin/recall <topic>` or `grep -ri "<topic>" journal/`; use `06-agent-digest.md` when recent activity, open threads, or episode pointers are useful. **Do not load the whole store.**\n\n' "$jn" "$(( jt / 4 ))"
  ...
  printf '> load and what it costs** — read a ROW, then open only the one file you need. The\n'
  printf '> on-demand index complements normal `agent_docs/` + active-directive continuity ([[memory]]).\n'
} > "${PAGE}"
