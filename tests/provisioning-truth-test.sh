...
grep -q 'imported_core_cts' "${cts}" && ok "imported CT compatibility map is explicit" || bad "missing imported CT map"
grep -q 'native_core_cts' "${cts}" && ok "native core CT map is explicit" || bad "missing native CT map"
printf '%s\n' "${native}" | grep -q '^resource "proxmox_virtual_environment_container" "core_ct"' \
...
grep -q 'scoped Authentik Applications/Providers' AGENTS.md \
  && ok "always-loaded trust table exposes Authentik T2 carve-out" || bad "Authentik T2 carve-out missing from AGENTS"
grep -q 'temporary bootstrap SSH key' runbooks/provision-vm.md \
...
printf '%s\n' "${current_directive}" | grep -q 'current_phase: 4' \
  && ok "SKY-024 points to the next unfinished phase" || bad "SKY-024 phase pointer is stale"
printf '%s\n' "${current_directive}" | grep -Eq '`tofu apply`|`tofu destroy`' \
